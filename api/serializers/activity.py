...
class ActivitySerializerBase(serializers.Serializer):
    ...
    parcours_doctoral = serializers.PrimaryKeyRelatedField(
        queryset=ParcoursDoctoral.objects.all(),
        write_only=True,
        required=False,
    )
    ...
    def __init__(self, *args, parcours_doctoral=None, child_classes=None, **kwargs):
...
class DoctoralTrainingActivitySerializer(serializers.Serializer):
    ...
    def __init__(self, *args, parcours_doctoral=None, only_classes=None, **kwargs):
        ...
        if data.get('parent'):
            if not isinstance(data['parent'], Activity):
                parent = get_object_or_404(
                    Activity.objects.filter(parcours_doctoral_id=data['parcours_doctoral']), uuid=data['parent']
                )
            else:
                parent = data['parent']
