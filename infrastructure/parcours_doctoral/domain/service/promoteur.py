...
from parcours_doctoral.ddd.domain.validator.exceptions import PromoteurNonTrouveException
...
class PromoteurTranslator(IPromoteurTranslator):
    ...
    def get_dto(cls, promoteur_id: 'PromoteurIdentity') -> 'PromoteurDTO':
        actor = ParcoursDoctoralSupervisionActor.objects.select_related('person__tutor', 'country').get(
    ...
    def est_externe(cls, promoteur_id: 'PromoteurIdentity') -> bool:
        return ParcoursDoctoralSupervisionActor.objects.get(type=ActorType.PROMOTER.name, uuid=promoteur_id.uuid).is_external
