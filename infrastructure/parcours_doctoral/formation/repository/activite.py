...
from parcours_doctoral.infrastructure.utils import (
    get_doctorate_training_acronym,
)
...
class ActiviteRepository(IActiviteRepository):
    ...
    def get_dto(cls, entity_id: 'ActiviteIdentity') -> ActiviteDTO:
        activity = Activity.objects.annotate_with_learning_year_info().select_related('parcours_doctoral', 'parent').get(uuid=entity_id.uuid)
    ...
    def lister_inscriptions_unites_enseignement(
        ...
        ).filter_by_learning_year(acronym=code_unite_enseignement, year=annee).annotate(
