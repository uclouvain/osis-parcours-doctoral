...
class PrivatePublicDefensesAPIViewTestCase(MockOsisDocumentMixin, APITestCase):
    ...
    def test_edit_with_valid_data(self):
        ...
        self.assertEqual(self.doctorate.defense_datetime, datetime.datetime.fromisoformat(self.data['date_heure_soutenance_publique']),)
        ...
        self.assertEqual(self.private_defense.datetime, datetime.datetime.fromisoformat(self.data['date_heure_defense_privee']),)
        self.assertEqual(self.private_defense.place, self.data['lieu_defense_privee'])
        self.assertEqual(self.private_defense.manuscript_submission_date, datetime.date.fromisoformat(self.data['date_envoi_manuscrit']),)

        historic_entries = HistoryEntry.objects.filter(object_uuid=self.doctorate.uuid)
        self.assertEqual(len(historic_entries), 1)
        self.assertCountEqual(historic_entries[0].tags, ["parcours_doctoral", "private-public-defenses", "status-changed"],)
        ...
        self.assertEqual(self.doctorate.defense_datetime, datetime.datetime.fromisoformat(new_data['date_heure_soutenance_publique']),)
        ...
        self.assertEqual(self.private_defense.datetime, datetime.datetime.fromisoformat(new_data['date_heure_defense_privee']),)
        self.assertEqual(self.private_defense.place, new_data['lieu_defense_privee'])
        self.assertEqual(self.private_defense.manuscript_submission_date, datetime.date.fromisoformat(new_data['date_envoi_manuscrit']),)
