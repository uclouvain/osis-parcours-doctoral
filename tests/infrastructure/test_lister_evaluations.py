...
class ListerEvaluationsTestCase(QueriesAssertionsMixin, TestCase):
    ...
    def test_list_with_assessments(self):
        ...
        activities.sort(key=lambda activity: activity.note)

        self.assertEqual(len(activities), 2)

        self.assertEqual(activities[0].uuid, str(first_assessment.uuid))
        self.assertEqual(activities[0].uuid_activite, str(first_assessment.course.uuid))
        self.assertEqual(activities[0].statut, first_assessment.status)
        self.assertEqual(activities[0].annee, 2023)
        self.assertEqual(activities[0].session, 1)
        self.assertEqual(
            activities[0].noma,
            first_assessment.course.parcours_doctoral.student.student_set.first().registration_id,
        )
        self.assertEqual(activities[0].code_unite_enseignement, 'UE1')
        self.assertEqual(activities[0].note, '15')
        self.assertEqual(activities[0].echeance_enseignant, datetime.date(2024, 1, 31))
        self.assertEqual(activities[0].est_desinscrit_tardivement, True)
        self.assertEqual(activities[0].est_inscrit_tardivement, False)

        self.assertEqual(activities[1].uuid, str(second_assessment.uuid))
        self.assertEqual(activities[1].uuid_activite, str(second_assessment.course.uuid))
        self.assertEqual(activities[1].statut, second_assessment.status)
        self.assertEqual(activities[1].annee, 2023)
        self.assertEqual(activities[1].session, 1)
        self.assertEqual(
            activities[1].noma,
            second_assessment.course.parcours_doctoral.student.student_set.first().registration_id,
        )
        self.assertEqual(activities[1].code_unite_enseignement, 'UE1')
        self.assertEqual(activities[1].note, '16')
        self.assertEqual(activities[1].echeance_enseignant, datetime.date(2024, 1, 31))
        self.assertEqual(activities[1].est_desinscrit_tardivement, False)
        self.assertEqual(activities[1].est_inscrit_tardivement, True)
