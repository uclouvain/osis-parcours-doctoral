...
class ManuscriptValidationViewWithAdreManagerTestCase(MockOsisDocumentMixin, TestCase):
    @classmethod
...
@override_settings(OSIS_DOCUMENT_BASE_URL='http://dummyurl')
class ManuscriptValidationViewWithScebManagerTestCase(MockOsisDocumentMixin, TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.academic_years = [AcademicYearFactory(year=year) for year in [2021, 2022]]
    ...
    def test_refuse_the_thesis(self):
        ...
        self.assertEqual(len(to_email_addresses), 1)
        self.assertEqual(to_email_addresses[0], self.doctorate.student.email)
        self.assertEqual(len(cc_email_addresses), 3)
        self.assertCountEqual(
            cc_email_addresses,
            [self.promoter.person.email, self.adre_manager.person.email, self.sceb_manager.person.email],
        )
