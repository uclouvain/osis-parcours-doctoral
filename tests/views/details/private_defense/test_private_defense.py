...
class PrivateDefenseDetailViewTestCase(MockOsisDocumentMixin, TestCase):
    ...
    def test_with_no_private_defense(self):
        ...
        previous_private_defenses = response.context.get('previous_private_defenses')
        supervision = response.context.get('supervision')

        self.assertIsNone(current_private_defense)
        self.assertEqual(previous_private_defenses, [])
    ...
    def test_with_several_private_defenses(self):
        ...
        current_private_defense: Optional[DefensePriveeDTO] = response.context.get('current_private_defense')
        previous_private_defenses: list[DefensePriveeDTO] = response.context.get('previous_private_defenses')
        ...
        self.assertEqual(len(previous_private_defenses), 1)
        self.assertEqual(previous_private_defenses[0].uuid, str(old_private_defense.uuid))
        self.assertEqual(previous_private_defenses[0].parcours_doctoral_uuid, (str(self.doctorate.uuid)))
        self.assertEqual(previous_private_defenses[0].est_active, False)
        self.assertEqual(previous_private_defenses[0].titre_these, self.doctorate.thesis_proposed_title)
        self.assertEqual(previous_private_defenses[0].date_heure, old_private_defense.datetime)
        self.assertEqual(previous_private_defenses[0].lieu, old_private_defense.place)
        self.assertEqual(
            previous_private_defenses[0].date_envoi_manuscrit, old_private_defense.manuscript_submission_date
        )
        self.assertEqual(previous_private_defenses[0].proces_verbal, old_private_defense.minutes)
        self.assertEqual(previous_private_defenses[0].canevas_proces_verbal, old_private_defense.minutes_canvas)
