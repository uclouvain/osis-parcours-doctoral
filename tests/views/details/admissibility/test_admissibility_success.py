...
class AdmissibilitySuccessViewTestCase(MockOsisDocumentMixin, TestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        cls.path = 'parcours_doctoral:admissibility:success'
    ...
    def test_post_success_form_with_invalid_data(self):
        self.client.force_login(self.manager.user)
        ...
        self.admissibility.decision_date = None
        ...
        self.assertIn(
            gettext(
                'Please be sure that the date of the admissibility have been filled in and that the admissibility '
                'minutes have been submitted in the application.'
            ),
            form.errors.get('__all__', []),
        )

        # No specified minutes
        self.admissibility.decision_date = datetime.date.today()
    ...
    def test_post_success_form_with_valid_data(self):
        self.client.force_login(self.manager.user)
        ...
        self.assertEqual(self.doctorate.status, ChoixStatutParcoursDoctoral.RECEVABILITE_REUSSIE.name)
        ...
        # Check a custom (tokens and language) notification has been sent to the candidate and to the jury members
