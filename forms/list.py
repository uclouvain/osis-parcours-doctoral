...
from base.forms.utils import autocomplete, FIELD_REQUIRED_MESSAGE
...
from parcours_doctoral.models import ParcoursDoctoralSupervisionActor, JuryMember
...
class ParcoursDoctorauxFilterForm(forms.Form):
    ...
    def __init__(self, user, load_labels=False, *args, **kwargs):
        ...
        if load_labels:
            student_global_id = self.data.get(self.add_prefix('matricule_etudiant'))
            if student_global_id:
                student = Person.objects.filter(global_id=student_global_id).first()
                if student:
            ...
            if jury_president_uuid:
                jury_president = JuryMember.objects.filter(uuid=jury_president_uuid).select_related('person').first()
                if jury_president:
