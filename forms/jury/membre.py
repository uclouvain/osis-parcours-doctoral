    ...
    def clean(self):
        if 'institution_principale' in self.cleaned_data:
            if self.cleaned_data[
                'institution_principale'
            ] == self.InstitutionPrincipaleChoices.UCL.name and not self.cleaned_data.get('matricule'):
                self.add_error('matricule', forms.ValidationError(self.fields['matricule'].error_messages['required']))
