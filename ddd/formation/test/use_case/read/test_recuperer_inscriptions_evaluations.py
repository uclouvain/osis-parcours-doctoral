...
class RecupererInscriptionsEvaluationsTestCase(TestCase):
    @classmethod
    ...
    def test_recuperer_inscriptions_evaluations(self):
        inscriptions: List[InscriptionEvaluationDTO] = self.message_bus.invoke(
            command=RecupererInscriptionsEvaluationsQuery(
                parcours_doctoral_uuid=str(uuid.uuid4()),
            )
        )

        self.assertEqual(len(inscriptions), 1)
        self.assertEqual(inscriptions[0].uuid, self.inscription.entity_id.uuid)
