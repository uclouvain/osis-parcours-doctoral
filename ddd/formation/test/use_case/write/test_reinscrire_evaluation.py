...
from parcours_doctoral.ddd.formation.commands import DesinscrireEvaluationCommand, ReinscrireEvaluationCommand
...
class DesinscrireEvaluationTestCase(TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.inscription = InscriptionEvaluation(
            ...
            statut=StatutInscriptionEvaluation.DESINSCRITE,
    ...
    def test_reinscrire_evaluation(self):
        cmd = ReinscrireEvaluationCommand(
            inscription_uuid=self.inscription.entity_id.uuid,
        )

        identite_inscription = self.message_bus.invoke(cmd)

        inscription = self.inscription_evaluation_repository.get_dto(identite_inscription)

        self.assertEqual(inscription.uuid, self.inscription.entity_id.uuid)
        self.assertEqual(inscription.statut, StatutInscriptionEvaluation.ACCEPTEE.name)
        self.assertEqual(inscription.session, self.inscription.session.name)
        self.assertEqual(inscription.inscription_tardive, self.inscription.inscription_tardive)
        self.assertEqual(inscription.est_acceptee, True)
        self.assertEqual(inscription.est_annulee, False)
