...
class TestAccepterTheseParAdreReference(SimpleTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.cmd = AccepterTheseParAdreCommand
    ...
    def setUp(self):
        ...
        self.adre = SignataireAutorisationDiffusionTheseFactory(
            entity_id__role=RoleActeur.ADRE,
            entity_id__matricule='123456',
        ...
        self.autorisation_diffusion_these: AutorisationDiffusionThese = AutorisationDiffusionTheseFactory(
            statut=ChoixStatutAutorisationDiffusionThese.DIFFUSION_VALIDEE_PROMOTEUR,
            signataires={RoleActeur.ADRE: self.adre},
        )
    ...
    def test_doit_generer_exception_si_acteur_non_invite(self):
        self.autorisation_diffusion_these.signataires = {
            RoleActeur.ADRE: SignataireAutorisationDiffusionTheseFactory(
                entity_id__role=RoleActeur.ADRE,
                entity_id__matricule='123456',
    ...
    def test_doit_modifier_statut_et_enregistrer_informations_acceptation(self):
        ...
        self.assertEqual(autorisation.statut, ChoixStatutAutorisationDiffusionThese.DIFFUSION_VALIDEE_ADRE)

        self.assertIsNone(autorisation.signataires.get(RoleActeur.PROMOTEUR))
        self.assertIsNotNone(autorisation.signataires.get(RoleActeur.SCEB))
        self.assertIsNotNone(autorisation.signataires.get(RoleActeur.ADRE))

        adre = autorisation.signataires.get(RoleActeur.ADRE)
        self.assertEqual(adre.entity_id.matricule, '123456')
        self.assertEqual(adre.entity_id.role, RoleActeur.ADRE)
        self.assertEqual(adre.signature.commentaire_interne, self.parametres_cmd['commentaire_interne'])
        self.assertEqual(adre.signature.etat, ChoixEtatSignature.APPROVED)
        self.assertEqual(adre.signature.commentaire_externe, self.parametres_cmd['commentaire_externe'])
        self.assertEqual(adre.signature.motif_refus, '')

        sceb = autorisation.signataires.get(RoleActeur.SCEB)
        self.assertEqual(sceb.entity_id.matricule, '012345')
        self.assertEqual(sceb.entity_id.role, RoleActeur.SCEB)
        self.assertEqual(sceb.signature.etat, ChoixEtatSignature.INVITED)
