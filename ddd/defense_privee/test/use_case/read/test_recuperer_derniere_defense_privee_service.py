...
from parcours_doctoral.ddd.defense_privee.test.factory.defense_privee import DefensePriveeFactory
...
class TestRecupererDerniereDefensePrivee(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.cmd = RecupererDerniereDefensePriveeQuery
    ...
    def test_should_pas_trouver_si_parcours_doctoral_inconnu(self):
        with self.assertRaises(DefensePriveeNonTrouveeException):
            self.message_bus.invoke(self.cmd(parcours_doctoral_uuid='inconnu'))

    def test_should_retourner_defense_privee(self):
        defense_privee: DefensePriveeDTO = self.message_bus.invoke(
            self.cmd(parcours_doctoral_uuid=self.defense_privee.parcours_doctoral_id.uuid)
        )
