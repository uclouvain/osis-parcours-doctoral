...
class _RecevabiliteIdentityFactory(factory.Factory):
    class Meta:
        model = RecevabiliteIdentity
        abstract = False

    uuid = factory.LazyFunction(lambda: str(uuid.uuid4()))


class RecevabiliteFactory(factory.Factory):
    class Meta:
        model = Recevabilite
        abstract = False

    entity_id = factory.SubFactory(_RecevabiliteIdentityFactory)
    parcours_doctoral_id = factory.SubFactory(_ParcoursDoctoralIdentityFactory)
    est_active = True
    date_decision = FuzzyNaiveDateTime(start_dt=datetime.datetime(2020, 1, 1))
    date_envoi_manuscrit = FuzzyDate(start_date=datetime.date(2020, 1, 1))
    avis_jury = factory.List(['file-0'])
