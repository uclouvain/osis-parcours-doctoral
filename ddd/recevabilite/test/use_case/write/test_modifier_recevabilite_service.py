...
class TestModifierRecevabilite(SimpleTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.cmd = ModifierRecevabiliteCommand
    ...
    def setUp(self):
        ...
        self.parametres_cmd = {
            ...
            'avis_jury': ['uuid-avis-jury'],
            'proces_verbal': ['uuid-proces-verbal'],
        }

    def test_should_soumettre_recevabilite_pour_premiere_fois(self):
        parcours_doctoral_id_resultat = self.message_bus.invoke(self.cmd(**self.parametres_cmd))

        recevabilite_mise_a_jour = self.recevabilite_repository.get_active(
        ...
        self.assertEqual(recevabilite_mise_a_jour.avis_jury, self.parametres_cmd['avis_jury'])
        self.assertEqual(recevabilite_mise_a_jour.proces_verbal, self.parametres_cmd['proces_verbal'])

        self.assertEqual(parcours_doctoral.titre_these_propose, self.parametres_cmd['titre_these'])

    def test_should_soumettre_recevabilite_pour_une_mise_a_jour(self):
        recevabilite = RecevabiliteFactory(parcours_doctoral_id=self.parcours_doctoral_repository.entities[0].entity_id)
