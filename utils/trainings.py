...
from parcours_doctoral.ddd.formation.domain.model.enums import StatutActivite, CategorieActivite, ChoixTypeEpreuve
...
def training_categories_stats(activities: List[Activity]):
    ...
    for category, category_activities in categories.items():
        category_submitted = sum(activity.ects for activity in category_activities if activity.status != StatutActivite.ACCEPTEE.name)
        category_validated = sum(activity.ects for activity in category_activities if activity.status == StatutActivite.ACCEPTEE.name)
