...
__all__ = [
    'CurrentLearningUnitYearAndClassesAutocomplete',
]

__namespace__ = False


class CurrentLearningUnitYearAndClassesAutocomplete(BaseLearningUnitYearAutoComplete):
    urlpatterns = 'current-learning-unit-years-and-classes'

    def get_list(self):
        annee = current_academic_year().year
        types_a_exclure = [LearningContainerYearType.EXTERNAL.name]
        if not self.q or not annee:
            return []

        cmd = LearningUnitAndPartimSearchCommand(
            annee_academique=annee, terme_de_recherche=self.q, types_a_exclure=types_a_exclure
        )
        result_unites_enseignement = message_bus_instance.invoke(cmd)

        cmd = SearchDetailClassesEffectivesCommand(
            annee=annee, terme_de_recherche=self.q, types_a_exclure=types_a_exclure
        )
